...
function Flow({ chat = false }: { chat?: boolean }) {
  ...
  return page === '/quote' ? <QuoteRequest /> : chat ? <Chatbot /> : <PriceCalculator />;
}
function mount(children: React.ReactNode) {
  return render(<LanguageProvider><QuoteProvider><SwitchLanguage />{children}<Snapshot /></QuoteProvider></LanguageProvider>);
}
beforeEach(() => { localStorage.clear(); navigation.push.mockReset(); });

it('translates the full service page and switches back to Mongolian', async () => {
  const { container } = mount(<ServicesDetail />);
  expect(screen.getByRole('heading', { name: 'Санаанаас production хүртэл' })).toBeInTheDocument();
  await userEvent.click(screen.getByText('Switch language'));
  expect(screen.getByRole('heading', { name: 'From idea to production' })).toBeInTheDocument();
...
it('preserves calculator selections and quote prefill across language changes', async () => {
  ...
  await user.type(screen.getByRole('textbox'), 'A customer order management website');
  await user.click(screen.getByRole('button', { name: 'Request a quote' }));
  expect(screen.getByRole('combobox', { name: /Project type/ })).toHaveTextContent('Website development');
  expect(JSON.parse(screen.getByTestId('quote').textContent!)).toMatchObject({ projectType: 'website', complexity: 'simple', features: ['api'], estimatedPrice: 4840000 });
  await user.click(screen.getByText('Switch language'));
  expect(screen.getByRole('combobox', { name: /Төслийн төрөл/ })).toHaveTextContent('Вэб сайт хөгжүүлэлт');
  expect(screen.getByLabelText(/Төслийн дэлгэрэнгүй тайлбар/)).toHaveValue('A customer order management website');
});
...
it('relocalizes existing validation and completes the quote UI in English', async () => {
  ...
  fireEvent.change(screen.getByLabelText(/Phone number/), { target: { value: '+976 99112233' } });
  const numbers = screen.getByText(/^\d+ \+ \d+ = \?$/).textContent!.match(/\d+/g)!.map(Number);
  fireEvent.change(screen.getByLabelText(/Human verification/), { target: { value: String(numbers[0] + numbers[1]) } });
  await user.click(screen.getByRole('button', { name: 'Send request' }));
  expect(screen.getByRole('button', { name: 'Sending…' })).toBeDisabled();
  expect(await screen.findByRole('heading', { name: 'Request received' }, { timeout: 2500 })).toBeInTheDocument();
});
...
it('switches chatbot language mid-conversation without changing the estimate or user text', async () => {
  ...
  await waitFor(() => expect(screen.getByText(/• Price — ₮800,000/)).toBeInTheDocument(), { timeout: 2000 });
  await user.click(screen.getByText('Switch language'));
  expect(screen.getByText(/• Үнэ — ₮800,000/)).toBeInTheDocument();
  await user.click(screen.getByRole('button', { name: 'Дэлгэрэнгүй үнийн санал авах' }));
  expect(JSON.parse(screen.getByTestId('quote').textContent!)).toMatchObject({ projectType: 'website', teamSize: '1', description: 'Custom features', estimatedPrice: 800000 });
  expect(screen.getByRole('combobox', { name: /Төслийн төрөл/ })).toHaveTextContent('Вэб сайт хөгжүүлэлт');
});

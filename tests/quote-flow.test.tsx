...
function mount() {
  return render(<QuoteProvider><Flow /><QuoteSnapshot /></QuoteProvider>);
}
const submit = () => screen.getAllByRole('button', { name: 'Үнийн санал авах' })[0];
...
describe('calculator → quote', () => {
  ...
  ])('calculates %s / %s and prefills the quote', async (project, complexity, hours, price) => {
    mount();
    const user = userEvent.setup();
    await user.click(screen.getByRole('radio', { name: new RegExp(`^${project}`) }));
    await user.click(screen.getByRole('radio', { name: new RegExp(`^${complexity}`) }));
    await user.type(screen.getByRole('textbox'), 'Харилцагчийн захиалга удирдах шинэ систем');
    await user.click(submit());
    expect(navigation.push).toHaveBeenCalledWith('/quote');
    const quote = JSON.parse(screen.getByTestId('quote').textContent!);
    expect(quote).toMatchObject({ projectType: project, complexity, estimatedHours: hours, estimatedPrice: price });
    expect(screen.getByLabelText(/Төслийн дэлгэрэнгүй тайлбар/)).toHaveValue(quote.description);
    expect(screen.getByRole('combobox', { name: /Төслийн төрөл/ })).toHaveTextContent(project);
  });
  ...
  it('adds feature hours after complexity and removes deselected features', async () => {
    ...
    await user.click(screen.getByRole('checkbox', { name: /Контент удирдлага/ }));
    await user.click(screen.getByRole('checkbox', { name: /API интеграци/ }));
    ...
    expect(JSON.parse(screen.getByTestId('quote').textContent!)).toMatchObject({
      features: ['Контент удирдлага (CMS)'], estimatedHours: 80, estimatedPrice: 7040000,
      timeline: 'urgent', teamSize: '2-3',
    });
    expect(screen.getByRole('combobox', { name: /Хугацаа/ })).toHaveTextContent('Яаралтай');
  });
  ...
  it('blocks missing selections and trimmed descriptions shorter than 20 characters', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'a'.repeat(20) } });
    await user.click(submit());
    expect(navigation.push).toHaveBeenCalledWith('/quote');
  });
  ...
  it('drops in-memory prefill on a fresh provider mount', async () => {
    ...
    await user.type(screen.getByRole('textbox'), 'a'.repeat(20));
    await user.click(submit());
    expect(screen.getByLabelText(/Төслийн дэлгэрэнгүй тайлбар/)).toHaveValue('a'.repeat(20));
    view.unmount();
    render(<QuoteProvider><QuoteRequest /></QuoteProvider>);
    expect(screen.getByLabelText(/Төслийн дэлгэрэнгүй тайлбар/)).toHaveValue('');
    expect(screen.getByRole('combobox', { name: /Төслийн төрөл/ })).toHaveTextContent('Төслийн төрлийг сонгоно уу');
  });

  it('rejects an empty quote and focuses the first invalid field', async () => {
    render(<QuoteProvider><QuoteRequest /></QuoteProvider>);
    await userEvent.click(screen.getByRole('button', { name: 'Хүсэлт илгээх' }));
